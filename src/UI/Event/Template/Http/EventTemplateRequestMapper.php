...
readonly class EventTemplateRequestMapper
{
    ...
    public function create(Request $request): CreateEventTemplateRequest
    {
        $data = $request->getPayload();

        return new CreateEventTemplateRequest(
            kind: $this->kind($data),
    ...
    public function update(string $id, Request $request): UpdateEventTemplateRequest
    {
        $data = $request->getPayload();

        return new UpdateEventTemplateRequest(
            id: $id,
            title: $this->requiredString($data, 'title'),
            content: $this->content($data),
            mediaUrl: $this->nullableString($data, 'mediaUrl'),
    ...
    private function kind(InputBag $data): EventScheduleKind
    {
        try {
            ...
            throw new BadRequestHttpException('Die Art der Vorlage ist ungültig.');
        }
    }
    ...
    /**
     * @param InputBag<string|int|float|bool|null> $data
     * @return list<EventTemplateActivity>
     */
    private function activities(InputBag $data): array
    {
        ...
        foreach ($activities as $position => $activity) {
            ...
            $mapped[] = new EventTemplateActivity(
    ...
    /**
     * @param InputBag<string|int|float|bool|null> $data
     * @return list<EventTemplateCallToAction>
     */
    private function callToActions(InputBag $data): array
    {
        ...
        foreach ($callToActions as $position => $callToAction) {
            ...
            $mapped[] = new EventTemplateCallToAction(
                id: $this->identifierGenerator->generate(),
                position: $position,
